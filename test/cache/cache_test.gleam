...
pub fn put_and_get_test() {
  ...
  // Cleanup
  let _ = cache.forget(pool, "test_key")
}

pub fn get_not_found_test() {
  let pool = setup_test_pool()

  cache.get(pool, "nonexistent_key_abc123")
  |> should.be_error()
...
pub fn put_overwrites_existing_test() {
  ...
  cache.put(pool, "overwrite_key", "first_value", 3600)
  |> should.be_ok()

  cache.put(pool, "overwrite_key", "second_value", 3600)
  |> should.be_ok()
  ...
  // Cleanup
  let _ = cache.forget(pool, "overwrite_key")
}
...
pub fn put_forever_test() {
  ...
  // Cleanup
  let _ = cache.forget(pool, "permanent_key")
}
...
pub fn forget_nonexistent_key_test() {
  ...
  cache.forget(pool, "never_existed_xyz789")
  |> should.be_ok()
}
...
pub fn has_existing_key_test() {
  ...
  cache.put(pool, "exists_key", "value", 3600)
  |> should.be_ok()

  cache.has(pool, "exists_key")
  |> should.equal(True)

  // Cleanup
  let _ = cache.forget(pool, "exists_key")
}

pub fn has_nonexistent_key_test() {
  let pool = setup_test_pool()

  cache.has(pool, "does_not_exist_456")
  |> should.equal(False)
}
...
pub fn pull_nonexistent_key_test() {
  ...
  cache.pull(pool, "nonexistent_pull_key")
  |> should.be_error()
  |> should.equal(NotFound)
}
...
pub fn increment_new_key_test() {
  ...
  cache.increment(pool, "counter_new", 1)
  |> should.be_ok()
  |> should.equal(1)

  // Cleanup
  let _ = cache.forget(pool, "counter_new")
}

pub fn increment_existing_key_test() {
  let pool = setup_test_pool()

  cache.increment(pool, "counter_inc", 1) |> should.be_ok()
  cache.increment(pool, "counter_inc", 1) |> should.be_ok()

  cache.increment(pool, "counter_inc", 1)
  |> should.be_ok()
  |> should.equal(3)

  // Cleanup
  let _ = cache.forget(pool, "counter_inc")
}

pub fn increment_by_amount_test() {
  let pool = setup_test_pool()

  cache.increment(pool, "counter_amt", 5)
  |> should.be_ok()
  |> should.equal(5)

  cache.increment(pool, "counter_amt", 10)
  |> should.be_ok()
  |> should.equal(15)

  // Cleanup
  let _ = cache.forget(pool, "counter_amt")
}

pub fn decrement_test() {
  let pool = setup_test_pool()

  cache.increment(pool, "counter_dec", 10) |> should.be_ok()

  cache.decrement(pool, "counter_dec", 3)
  |> should.be_ok()
  |> should.equal(7)

  // Cleanup
  let _ = cache.forget(pool, "counter_dec")
}

pub fn decrement_below_zero_test() {
  let pool = setup_test_pool()

  cache.decrement(pool, "counter_neg", 5)
  |> should.be_ok()
  |> should.equal(-5)

  // Cleanup
  let _ = cache.forget(pool, "counter_neg")
}
...
pub fn put_json_and_get_json_test() {
  ...
  cache.put_json(pool, "json_user", user, user_encoder, 3600)
  |> should.be_ok()

  cache.get_json(pool, "json_user", user_decoder())
  |> should.be_ok()
  |> should.equal(user)

  // Cleanup
  let _ = cache.forget(pool, "json_user")
}
...
pub fn put_json_forever_test() {
  ...
  cache.put_json_forever(pool, "permanent_json_user", user, user_encoder)
  |> should.be_ok()

  cache.get_json(pool, "permanent_json_user", user_decoder())
  |> should.be_ok()
  |> should.equal(user)

  // Cleanup
  let _ = cache.forget(pool, "permanent_json_user")
}
...
pub fn get_json_invalid_format_test() {
  ...
  cache.put(pool, "invalid_json", "not valid json", 3600)
  |> should.be_ok()

  cache.get_json(pool, "invalid_json", user_decoder())
  |> should.be_error()
...
pub fn remember_returns_cached_value_test() {
  ...
  cache.remember(pool, "cached_remember", 3600, fn() { "computed_value" })
...
pub fn remember_computes_when_missing_test() {
  ...
  cache.remember(pool, "missing_remember", 3600, fn() { "computed_value" })
  |> should.equal("computed_value")

  // Value should now be cached
  cache.get(pool, "missing_remember")
  |> should.be_ok()
  |> should.equal("computed_value")
...
pub fn remember_forever_test() {
  ...
  cache.remember_forever(pool, "permanent_remember", fn() { "computed" })
  |> should.equal("computed")

  cache.get(pool, "permanent_remember")
  |> should.be_ok()
  |> should.equal("computed")
...
pub fn remember_json_computes_when_missing_test() {
  ...
  cache.get_json(pool, "new_user_json", user_decoder())
  |> should.be_ok()
  |> should.equal(user)

  // Cleanup
  let _ = cache.forget(pool, "new_user_json")
}
...
pub fn flush_removes_all_pool_keys_test() {
  ...
  // Add multiple keys
  cache.put(pool, "flush_key1", "value1", 3600) |> should.be_ok()
  cache.put(pool, "flush_key2", "value2", 3600) |> should.be_ok()
  cache.put(pool, "flush_key3", "value3", 3600) |> should.be_ok()

  // Verify they exist
  cache.has(pool, "flush_key1") |> should.equal(True)
  cache.has(pool, "flush_key2") |> should.equal(True)
  cache.has(pool, "flush_key3") |> should.equal(True)
  ...
  // Verify they're gone
  cache.has(pool, "flush_key1") |> should.equal(False)
  cache.has(pool, "flush_key2") |> should.equal(False)
  cache.has(pool, "flush_key3") |> should.equal(False)
}
...
pub fn flush_only_affects_pool_prefix_test() {
  ...
  cache.put(pool, "test_flush_key", "test_value", 3600) |> should.be_ok()
  cache.put(other_pool, "other_flush_key", "other_value", 3600)
  |> should.be_ok()
  ...
  cache.has(pool, "test_flush_key") |> should.equal(False)

  // Other pool's key should still exist
  cache.has(other_pool, "other_flush_key") |> should.equal(True)

  // Cleanup
  let _ = cache.forget(other_pool, "other_flush_key")
}
